...
- **Night Scrap Bonus (Task #10)**: Kills (zombie + nest) and evacuations performed while `nightFactor(state.tick) > 0.5` are tallied into `state.nightKills` / `state.nightEvacuations`. `computeRunResult` adds `nightBonus = nightKills*2 + nightEvacuations*5` (formula corrected from original `*1` to match the ICE-8 spec doc — see "Night-bonus formula locked in" below) to `scrapEarned` and exposes it via `RunResult.nightBonus` + `scrapBreakdown.nightBonus`. WSS2MetaShell results screen renders a conditional "Night bonus" row when `nightBonus > 0`. No AI/zombie behavior changes — pure economy reward to encourage night risk.
...
- **Night-bonus formula locked in**: The night-bonus math is now extracted into `export function computeNightBonus(nightKills, nightEvacuations, scrapMultiplier)` — a single source of truth used by both the run-end breakdown and the live HUD pill. The formula is `round((nightKills × 2 + nightEvacuations × 5) × scrapMultiplier)`. The "× 2" weight on night kills aligns with the ICE-8 spec doc and the design intent (night is more dangerous, so night kills earn more than day kills which only collect the per-kill +1). A dev-only self-test runs at module load (gated by `import.meta.env.DEV`) with seven explicit cases (zero, kills only, evacs only, mixed, multiplier scaling, negative-clamp, zero-multiplier). If anyone changes the formula without updating the cases, the dev console throws on the next reload.
- **Starter Cache perk (5th perk, LIVE)**: New emerald-accented perk in the meta shop with a `Package` icon. Each level grants `PERK_STARTER_CACHE_PER_LEVEL = 5` flat scrap added to the run-end reward, applied AFTER (not multiplied by) the Scrap Magnet multiplier — so the two perks stack additively as a flat baseline + percentage on top. 3 levels with costs `[50, 120, 240]`. The run-end results screen shows a conditional "Starter Cache (lvl X × 5)" row only when the bonus is greater than zero, and the row's level is derived from the breakdown value itself (not from `meta`) so the results screen never crashes if `meta` isn't in scope. Old localStorage saves missing the `starterCache` key are auto-backfilled to 0 by the existing `clampPerks({ ...EMPTY_PERKS, ...parsed.perks })` merge in `loadMeta()`.
