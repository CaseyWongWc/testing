...
type EntityKind    = "survivor" | "zombie" | "nest" | "portal" | "switch";
...
interface Survivor extends BaseEntity {
  ...
  weapon: Weapon; attackCooldown: number;
...
type AnyEntity = Survivor | Zombie | CorruptionNest | RiftPortal | ObjectiveSwitch;
...
interface LogEntry { tick: number; text: string; type: "combat"|"death"|"spawn"|"info"|"warn"|"objective"; }
...
const brainWeapon = (b: BrainType): Weapon => ({
  ...WEAPONS[{ aggressive:"shotgun", balanced:"pistol", cautious:"knife", survivalist:"bat", money:"rifle" }[b]],
});
...
function spawnEntities(settings: GameSettings, rand: () => number): AnyEntity[] {
  ...
  // Survivors — cluster around center
  ...
  // Objective switches — placed far from center and each other
  ...
  for (let i = 0; i < nestCount; i++) {
    ...
    entities.push({
      ...
      pos: { x: nx, y: ny }, health: 100, maxHealth: 100,
      spawnCooldown: Math.floor(rand() * 100),
      maxCooldown: NEST_COOLDOWN + Math.floor(rand() * 100),
    } as CorruptionNest);
  }

  // Initial zombies — FIX: spawn far enough that alertRadius(9) won't catch center
  const minSpawnDist = ZOMBIE_ALERT + 4; // at least 13 tiles from center
  for (let i = 0; i < zombieCount; i++) {
    const angle = rand() * Math.PI * 2;
    const r = minSpawnDist + rand() * (size * 0.5 - minSpawnDist - 2);
    const clampedR = Math.max(minSpawnDist, Math.min(size * 0.48, r));
    entities.push({
      id: nextId++, kind: "zombie", faction: "HOSTILE", dead: false, armor: 0,
      pos: {
        x: Math.max(1, Math.min(size - 1, half + Math.cos(angle) * clampedR)),
        y: Math.max(1, Math.min(size - 1, half + Math.sin(angle) * clampedR)),
      },
...
const FLEE_THRESH: Record<BrainType, number> = {
  aggressive:0.15, balanced:0.35, cautious:0.55, survivalist:0.4, money:0.3,
};
...
function chooseSurvivorAI(
  ...
  // 1 — FLEE
  if (hp < FLEE_THRESH[s.brain] && enemies.length > 0) {
  ...
  // 2 — FIGHT (move toward nearest hostile if in weapon range area)
  if (nearest && Math.hypot(nearest.pos.x - s.pos.x, nearest.pos.y - s.pos.y) <= Math.max(s.weapon.range, THREAT_DIST[s.brain])) {
    return { state:"fighting", target:{ ...nearest.pos } };
  }

  // 3 — EVACUATE (portal open, go stand on it)
  if (portalOpen && !s.evacuated) {
    return { state:"evacuating", target:{ ...portalPos } };
  }

  // 4 — ACTIVATE objective (go to current switch)
  if (currentObjPos && !portalOpen) {
    return { state:"activating", target:{ ...currentObjPos } };
  }

  // 5 — SCAVENGE (random walk)
...
function tickSurvivor(
  ...
  if (s.targetPos && aiState !== "fighting") {
    ...
    if (dist > (aiState === "activating" || aiState === "evacuating" ? 0.5 : 0.3)) {
      const baseSpeed = aiState === "fleeing" ? 0.05 : aiState === "evacuating" ? 0.04 : 0.03;
...
function tickZombie(z: Zombie, state: GameState): Zombie {
  ...
  // Find nearest survivor WITHIN alertRadius only
...
function resolveCombat(
  ...
  for (const e of entities) {
    ...
    if (best) {
      ...
      if (best.health <= 0) { best.dead=true; s.kills++; addLog(`${s.name} killed a zombie.`,"death"); }
    }
  }
...
function runTick(state: GameState): GameState {
  ...
  // Gather useful refs
  const portal = postCombat.find(e=>e.kind==="portal") as RiftPortal;
  const portalOpen = !portal.sealed;
  const switches = postCombat.filter(e=>e.kind==="switch") as ObjectiveSwitch[];
  ...
  for (const sw of switches) {
    ...
    if (holdersNearby > 0) {
      ...
      if (sw.holdProgress >= sw.holdRequired) {
        ...
        if (allDone) {
          // Unseal portal
          const p = postCombat.find(e=>e.kind==="portal") as RiftPortal;
  ...
  const stateForTick = { ...state, entities:postCombat, noiseEvents:noise };

  for (const e of postCombat) {
    if (e.dead) continue;
    if (e.kind === "survivor") {
      moved.push(tickSurvivor(e as Survivor, stateForTick, currentObjPos, {
        x:(postCombat.find(en=>en.kind==="portal") as RiftPortal)?.pos.x ?? 15,
        y:(postCombat.find(en=>en.kind==="portal") as RiftPortal)?.pos.y ?? 15,
      }, portalOpen));
  ...
  // Check evacuation — survivors within EVAC_RADIUS of open portal
  const portalEntity = moved.find(e=>e.kind==="portal") as RiftPortal;
  if (portalEntity && !portalEntity.sealed) {
    for (const e of moved) {
      if (e.kind !== "survivor" || e.dead) continue;
      const s = e as Survivor;
      if (!s.evacuated && Math.hypot(s.pos.x-portalEntity.pos.x, s.pos.y-portalEntity.pos.y) < EVAC_RADIUS) {
        s.evacuated = true;
  ...
  const evacuated = aliveSurvivors.filter(s=>s.evacuated).length;
  let winState: WinState = "playing";
  if (aliveSurvivors.length === 0) {
    winState = "lost";
    addLog("💀 GAME OVER — All survivors eliminated.", "death");
  } else if (aliveSurvivors.length > 0 && evacuated === aliveSurvivors.length && !portalEntity.sealed) {
    winState = "won";
...
function renderWorld(
  ...
  for (const noise of state.noiseEvents) {
    ...
    const prog=1-noise.ticksLeft/noise.maxTicks;
    ctx.beginPath(); ctx.arc(px,py,noise.radius*prog*ts,0,Math.PI*2);
  ...
  for (const ef of state.attackEffects) {
    ...
    const tx=(ef.to.x-camX)*ts,   ty=(ef.to.y-camY)*ts;
  ...
  // Draw compass arrows on all living survivors
  ...
  for (const e of state.entities) {
    ...
    if (e.kind === "switch") {
      ...
      if (isActive) {
        ...
        ctx.fillStyle=objColor+(Math.floor(pulse*0.3*255).toString(16).padStart(2,"0"));
        ctx.fill();
      }
      // Body
      ctx.beginPath(); ctx.arc(px,py,r,0,Math.PI*2);
      ctx.fillStyle = sw.activated ? "#44aa44" : isActive ? objColor : "#555";
      ctx.fill();
      ctx.strokeStyle = sw.activated ? "#88ff88" : isActive ? objColor : "#888";
      ...
      // Open portal — dramatic glow
      if (!p.sealed) {
        for (let ring = 3; ring >= 1; ring--) {
          ctx.beginPath(); ctx.arc(px,py,r*ring*1.1,0,Math.PI*2);
          ctx.fillStyle=`rgba(100,220,255,${pulse*0.08*ring})`;
          ctx.fill();
        }
        // Spinning ring
        const spinAngle = (now/800) % (Math.PI*2);
        ctx.beginPath();
        ctx.arc(px,py,r*1.6,spinAngle,spinAngle+Math.PI*1.2);
      ...
      ctx.beginPath(); ctx.arc(px,py,r,0,Math.PI*2);
      ctx.fillStyle = p.sealed ? "#334" : "#88ffff";
      ctx.fill();
      ctx.strokeStyle = p.sealed ? "#668" : "#aaffff";
      ctx.lineWidth=2; ctx.stroke();
      ...
      ctx.fillStyle="#aa44ff"; ctx.fillRect(bx,by,bw*(e.health/e.maxHealth),bh);
      ...
      const stateColor = s.aiState==="fighting"?"#ff4400":s.aiState==="fleeing"?"#ffff00":s.aiState==="activating"?"#ff9900":s.aiState==="evacuating"?"#88ffff":"#44ff88";
      // state ring
  ...
  // Draw evacuated survivors as sparkles on portal
  ...
  if (evacuatedCount > 0 && portal) {
    ...
    if (tile2 && tile2.fogState !== "hidden") {
      ...
      ctx.fillText(`+${evacuatedCount}✦`, ppx, ppy+ts*0.5);
    }
  }
}
...
function initGame(settings: GameSettings): GameState {
  ...
  return {
    ...
    log:[{tick:0,text:"World generated. Find the objectives.",type:"info"}],
...
const WSSPhase2: React.FC = () => {
  ...
  const [counts,      setCounts]      = useState({survivors:0,zombies:0,nests:0,evacuated:0});
  ...
  const handleReset=()=>{
    ...
    cameraRef.current={...cameraRef.current,
      ...
      y:half-(canvas?canvas.height/(2*ts):10), followId:null};
    syncUI();
  };
  ...
  const logColors:Record<LogEntry["type"],string>={
    ...
    info:"text-gray-400", warn:"text-orange-400", objective:"text-cyan-400",
  };
  const survivedCount = stateRef.current.entities.filter(e=>e.kind==="survivor"&&!e.dead).length;
  const evacuatedCount = (stateRef.current.entities.filter(e=>e.kind==="survivor"&&!e.dead) as Survivor[]).filter(s=>s.evacuated).length;
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-950 text-gray-100 font-mono overflow-hidden select-none">
      ...
      <div className="flex items-center gap-2 px-3 py-1.5 bg-gray-900 border-b border-gray-700 shrink-0 flex-wrap">
        ...
        <span className="text-purple-400 text-xs">PHASE 2</span>
        <span className="text-gray-700">│</span>
        <span className="text-yellow-400 text-xs">T:{tick.toLocaleString()}</span>

        {/* Objective progress mini indicators */}
        <span className="text-gray-700">│</span>
      ...
      <div className="flex flex-1 overflow-hidden">
                    ...
                    {sw.idx===objState.currentIdx&&!sw.activated&&sw.holdProgress>0&&(
                      <div className="mt-1 h-1.5 bg-gray-700 rounded">
                        <div className="h-1.5 rounded" style={{
                          width:`${(sw.holdProgress/sw.holdRequired)*100}%`,
                          backgroundColor: OBJ_COLORS[sw.idx%OBJ_COLORS.length],
                        }}/>
                      </div>
                    )}
                  </div>
              ...
              {survivors.map((s,i)=>(
                <div key={s.id}
                  ...
                  {!s.evacuated&&<>
                    ...
                    <div className={`text-xs mb-1 ${s.aiState==="fighting"?"text-red-400":s.aiState==="fleeing"?"text-yellow-400":s.aiState==="activating"?"text-orange-400":s.aiState==="evacuating"?"text-cyan-400":"text-green-400"}`}>
                      {s.aiState.toUpperCase()} · {s.kills}k
                    </div>
                    {[
                      {lbl:"HP",val:s.health,max:s.maxHealth,cls:"bg-red-500"},
        ...
        <div className="flex-1 relative overflow-hidden bg-black">
              ...
              <div className="text-center border border-cyan-500 bg-gray-950/95 px-12 py-10 rounded-2xl shadow-2xl">
                ...
                <div className="grid grid-cols-3 gap-4 mb-6 text-center">
                  <div className="bg-gray-800 rounded p-3">
                    <div className="text-cyan-300 text-xl font-bold">{survivedCount}</div>
                    <div className="text-gray-500 text-xs">Survived</div>
                  </div>
                  <div className="bg-gray-800 rounded p-3">
                    <div className="text-yellow-400 text-xl font-bold">{(winState==="won"?stateRef.current.winTick:tick).toLocaleString()}</div>
                    <div className="text-gray-500 text-xs">Ticks</div>
                  </div>
                  <div className="bg-gray-800 rounded p-3">
                    <div className="text-green-400 text-xl font-bold">3/3</div>
                    <div className="text-gray-500 text-xs">Objectives</div>
                  </div>
                </div>
                <button onClick={handleReset}
                  className="px-8 py-3 bg-cyan-800 hover:bg-cyan-700 border border-cyan-400 rounded-lg text-cyan-100 font-bold text-sm transition-colors">
                  Play Again
                </button>
              </div>
            </div>
            ...
            <div className="absolute inset-0 flex items-center justify-center bg-black/75 z-10">
              <div className="text-center border border-red-700 bg-gray-950/95 px-12 py-10 rounded-2xl shadow-2xl">
                ...
                <div className="grid grid-cols-2 gap-4 mb-6 text-center">
                  <div className="bg-gray-800 rounded p-3">
                    <div className="text-green-400 text-xl font-bold">{objState.switches.filter(s=>s.activated).length}/3</div>
                    <div className="text-gray-500 text-xs">Objectives done</div>
                  </div>
                  <div className="bg-gray-800 rounded p-3">
                    <div className="text-red-400 text-xl font-bold">{stateRef.current.entities.filter(e=>e.kind==="survivor"&&!e.dead&&(e as Survivor).kills).reduce((a,e)=>a+(e as Survivor).kills,0)}</div>
                    <div className="text-gray-500 text-xs">Total kills</div>
                  </div>
                </div>
                <button onClick={handleReset}
                  className="px-8 py-3 bg-red-900 hover:bg-red-800 border border-red-500 rounded-lg text-red-100 font-bold text-sm transition-colors">
                  Play Again
                </button>
              </div>
            </div>
          ...
          <div className="absolute top-3 right-3 flex flex-col gap-2 pointer-events-none">
            <div className={`px-3 py-1.5 rounded border text-xs font-bold backdrop-blur-sm transition-all ${objState.portalOpen?"bg-cyan-900/80 border-cyan-400 text-cyan-300 animate-pulse":"bg-gray-900/80 border-gray-600 text-gray-400"}`}>
            ...
            <div className="bg-gray-900/80 border border-gray-700 rounded px-3 py-2 text-xs space-y-1 backdrop-blur-sm">
              <div className="flex items-center gap-1.5 text-gray-300"><Activity className="w-3 h-3 text-green-400"/>{counts.survivors} alive · {counts.evacuated} evacuated</div>
              <div className="flex items-center gap-1.5 text-gray-300"><Skull className="w-3 h-3 text-red-400"/>{counts.zombies} zombies</div>
              <div className="flex items-center gap-1.5 text-gray-300"><Zap className="w-3 h-3 text-purple-400"/>{counts.nests} nests</div>
            </div>
          </div>
          ...
          {/* Bottom hint */}
          {winState==="playing"&&cameraMode!=="free"&&!objState.portalOpen&&objState.currentIdx<3&&(
            <div className="absolute bottom-3 left-1/2 -translate-x-1/2 text-xs text-gray-600 pointer-events-none">
              Follow dashed arrows → Objective {objState.currentIdx+1} · Hold for {HOLD_REQUIRED} ticks to activate
            </div>
          )}
        </div>
      </div>
